...
<div class="container">
  <form action="{{ route('user.apartment.update', $apartment->id) }}" method="post" enctype='multipart/form-data'>
      ...
      <div class="mb-3">
          ...
          <input type="url" class="form-control @error('cover_img') is-invalid @enderror" name='cover_img' id="cover_img" value="{{$apartment->cover_img}}" aria-describedby="emailHelp">
          @error('cover_img')
            <span class="invalid-feedback">
              <strong>{{ $message }}</strong>
            </span>
